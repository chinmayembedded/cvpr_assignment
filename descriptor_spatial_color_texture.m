function F = descriptor_spatial_color_texture(img, row_size, col_size, bins, thresh)
...
for i = 1:row_size
    for j=1:col_size
        ...
        gray_img = grid_img(:,:,1) * 0.3 + grid_img(:,:,2) * 0.59 + grid_img(:,:,3) * 0.11;
   
        % Edge magnitude and edge direction
        blur_img = conv2(gray_img, blur_filter, 'same');
        ...
        dst = [dst hist rgb_vals(1) rgb_vals(2) rgb_vals(3)];
    end 
end
F=dst;
return;
